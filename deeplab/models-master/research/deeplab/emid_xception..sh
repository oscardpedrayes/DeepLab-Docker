...
DATASET='emid-avi3-multi4-night' #'emid-avi3-multi4-night' #emid #-avi-multi'
LEARNING_RATE=0.000005
BATCH_SIZE=10
IMAGES_TRAINING=428 #428 #970 #634 #764 #3750 #2250 #1500 #1125 #634 #1125
EPOCHS=300
...
python "${WORK_DIR}"/runEval.py "${EXP_NAME}" "${MODEL_NAME}" "train" "${DATASET}" "${DATASET_DIR}/" "${TF_DATASET}" "${EXP_FOLDER}" 
python "${WORK_DIR}"/getMetrics.py "${EVAL_LOGDIR}/train/" "${METRICS_LOGDIR}/train/" 
python "${WORK_DIR}"/runEval.py "${EXP_NAME}" "${MODEL_NAME}" "test" "${DATASET}" "${DATASET_DIR}/" "${TF_DATASET}" "${EXP_FOLDER}" 
python "${WORK_DIR}"/getMetrics.py "${EVAL_LOGDIR}/test/" "${METRICS_LOGDIR}/test/" 
...
python "${WORK_DIR}"/detect.py "${EXP_NAME}" "${EXPORT_DIR}" '/datasets/EMID/20220121/AVI_NIGHT/test/RGB/' "${VIS_LOGDIR}/detections/" 
...
python "${WORK_DIR}"/getConfusionMatrix.py "${METRICS_LOGDIR}" '/datasets/EMID/20220121/AVI_NIGHT/test/masks_gray_1channel/' "${VIS_LOGDIR}/detections/"