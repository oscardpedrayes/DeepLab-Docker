...
flags.DEFINE_list('eval_crop_size', '513,385',
...
def metric_loss(labels, predict):
  #class_weights=[0.6182,3.9584,0.6904,11.2659,1.8127,0.6729, 0.0000]
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    metric_map['eval/globalaccuracy'] = tf.metrics.accuracy(
    ...
    metric_map['eval-loss/eval_loss'] = metric_loss(one_hot_labels, one_hot_predictions)
        #weights= [0.6182,3.9584,0.6904,11.2659,1.8127,0.6729, 0.0000])

    for c in range(num_classes):
      predictions_tag_c = '%s_class_%d' % (predictions_tag, c)
      recall_tag_c = '%s_class_%d' % ('recall', c)
      precision_tag_c = '%s_class_%d' % ('precision', c)
      ...
      metric_map['eval/%s' % recall_tag_c] = (recall, tp_fp_fn_op) 
      metric_map['eval/%s' % precision_tag_c] = (precision, tp_fp_fn_op)             
