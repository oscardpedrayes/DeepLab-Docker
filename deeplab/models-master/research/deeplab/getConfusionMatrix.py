...
def toGrayscale(img, width, height):
    ...
    for i in range(height):
        for j in range(width):  
            ...
            #if pixel == [119, 11, 32]:      color = 0
            #elif pixel == [0, 60, 100]:       color = 1
            #elif pixel == [150, 100, 100]:    color = 2
            #elif pixel == [255, 74, 74]:      color = 3
            #elif pixel == [0, 0, 142]:        color = 4
            #elif pixel == [70, 70, 70]:       color = 5
            #else:                           
            #    color = 255
            #    print('wo: ', pixel)
            if pixel == [119, 11, 32]:   color = 0
            if pixel == [150, 100, 100]: color = 1
            if pixel == [70, 70, 70]:    color = 2
            if pixel == [0, 0, 142]:     color = 3
            if pixel == [0, 60, 100]:    color = 4

            #if pixel == [119, 11, 32]:   color = 0
            #if pixel == [111, 74, 0]:    color = 1
            #if pixel == [0, 0, 142]:     color = 2
            #if pixel == [0, 60, 100]:    color = 3
...
for _, _, files in os.walk(GROUNDTRUTH_PATH, topdown=False):
    for name in files:
        if os.path.isfile(PREDICTED_PATH + name): #.replace('_gt','_image')):
            ...
            #gtImg = toGrayscale(gtImg, width, height)
            gtImg = gtImg.reshape(height*width)

            predImg = np.array(predImage)
            predImg = toGrayscale(predImg, width, height)
            predImg = predImg.reshape(height*width)

            # Calculate Confusion Matrix
            tmp_cm = tf.math.confusion_matrix(gtImg, predImg, num_classes=5)
            if sumCM == None:
                sumCM = tmp_cm
            else:
                sumCM = sumCM + tmp_cm
